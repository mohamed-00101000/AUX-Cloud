...
//	// Print/log status or toggle debug pins to inspect bits
//	// e.g. use UART printf(status) or light different LEDs
//	uint8_t tx_ds = status & 0x20;
	uint8_t tx_ds = nrf24l01p_get_status(); // read STATUS register
	tx_ds &= 0x20;                         // keep only bit 5 (0x20)
//2
    if(tx_ds)
    {   
        // TX_DS
